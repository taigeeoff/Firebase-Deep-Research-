...
export class A2AServer {
  ...
  card: schema.AgentCard | undefined;
  ...
  //   req: NextRequest,
  //   res: NextResponse,
  //   next: Function // eslint-disable-line @typescript-eslint/no-unused-vars
  // ) => {
  ...
  //       err
  //     );
  //     // Ensure the response is ended if it wasn't already -  This might not be directly applicable in Next.js
  //     if (!res.writableEnded) {
  //       res.end();
  //     }
  //     return;
  ...
  //       "status" in err &&
  //       err.status === 400
  //     ) { // This part might need adjustment based on how Next.js handles body parsing errors
  //       responseError = this.normalizeError(
  ...
  //   return NextResponse.json(responseError); // Use NextResponse.json
  // };

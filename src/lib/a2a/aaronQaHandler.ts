 ...
 */
export async function* virtualCustomerEngineerHandler(
  context: TaskContext
): AsyncGenerator<TaskYieldUpdate> {
  console.log(
    `[Virtual CE] Processing task ${context.task.id} with state ${context.task.status.state}`
  );
  ...
  console.log("Task Context in handler: ", context)
  ...
  if (messages.length === 0) {
    console.warn(
      `[Virtual CE] No valid text messages found in history for task ${context.task.id}. Cannot proceed.`
    );
    yield {
      state: "failed",
      message: {
  ...
  const userQueryTextPart = context.userMessage?.parts[0] as TextPart;
  const userQuery = userQueryTextPart.text as string | undefined;
  ...
  try {
    ...
    if (finalStateLine === "COMPLETED") {
      ...
      console.warn(
        `[Virtual CE] Unexpected final state line from prompt: ${finalStateLine}. Defaulting to 'completed'.`
      );
    ...
    console.log(
      `[Virtual CE] Task ${context.task.id} finished with state: ${finalState}`
    );
  } catch (error: any) {
    console.error(
      `[Virtual CE] Error processing task ${context.task.id}:`,
      error
    );
